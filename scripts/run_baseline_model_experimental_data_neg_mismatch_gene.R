...
# Gasperini et al. in 2019, but scrambles the gene for each enhancer, to generate one form of
# negative controls. This program was written by Karthik Guruvayurappan.
...
for (i in 1:nrow(enhancer.gene.pairs)) {
    ...
    gene <- sample(enhancer.gene.pairs$ENSG, 1)
...
write.csv(
    pvalue.table,
    '/iblm/netapp/data1/external/Gasperini2019/processed/23_01_12_enhancer_gene_pairs_suppl_table_2_baseline_pseudocount_model_neg_mismatch_gene.csv',
    row.names = FALSE
)
