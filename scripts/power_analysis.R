...
load_counts <- function(lambda, effect.sizes, neg = FALSE) {
	...
	for (j in 1:length(effect.sizes)) {
		set.name <- paste0("lambda", args$lambda, "_size", effect.sizes[j])
		if (neg) {
			set.name <- paste0("lambda", args$lambda, "_size-NEG", effect.sizes[j])
		}
...
if (args$test) {
	genes.to.test <- genes.to.test[1:5]
...
if (!is.null(args$lambda)) {
	...
	print(head(power.plotdf))
	print(dim(power.plotdf))
	print(tail(power.plotdf))
