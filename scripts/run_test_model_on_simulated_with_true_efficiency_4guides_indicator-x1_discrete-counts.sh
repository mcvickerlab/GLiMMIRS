...
outdir=/iblm/netapp/data1/jezhou/crisprQTL/sim_performance_true_efficiency_4guides_indic-x1_disc-counts_targeting
mkdir -p $outdir
...
--targeting --out $outdir --d 4 --x1 indicator --counts discrete

# message the user on slack if possible
exit_code="$?"
if command -v 'slack' &>/dev/null; then
    if [ "$exit_code" -eq 0 ]; then
		slack "test_model_on_simulated_with_true_efficiency using 4 guides, indicator x1, discrete counts finished successfully" &>/dev/null
	else
		slack "test_model_on_simulated_with_true_efficiency using 4 guides, indicator x1, discrete counts exited with error code $exit_code"
	fi
fi
exit "$exit_code"
