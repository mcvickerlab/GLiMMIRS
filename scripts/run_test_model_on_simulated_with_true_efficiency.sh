...
outdir=/iblm/netapp/data1/jezhou/crisprQTL/sim_performance_true_efficiency_4guides_targeting
mkdir -p $outdir
...
--targeting --out $outdir 

# message the user on slack if possible
exit_code="$?"
if command -v 'slack' &>/dev/null; then
    if [ "$exit_code" -eq 0 ]; then
		slack "test_model_on_simulated_with_true_efficiency finished successfully" &>/dev/null
	else
		slack "test_model_on_simulated_with_true_efficiency exited with error code $exit_code"
	fi
fi
exit "$exit_code"
