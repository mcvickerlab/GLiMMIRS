...
### evaluating all genes (including non-targeted)
### using continuous X1 values
### using counts simulated with discrete X1 values

# outdir=$HOME/crisprqtl_sim/sim_data
outdir=/iblm/netapp/data1/jezhou/crisprQTL/sim_performance_true_efficiency_4guides_cont-x1_disc-counts_ALL-GENES
mkdir -p $outdir
...
--out $outdir --d 4 --x1 continuous --counts discrete
