# From the at-scale enhancer-enhancer interaction analysis, there were 4
# interaction coefficients which were significant. In this script, I shuffle
# the assignment of cells and their gene counts (holding guide assignments
# constant). This will generate a null distribution of interaction coefficients
# from which a p-value can be determined.
#
...
for (i in 1:nrow(significant.interactions)) {
    ...
    for (j in 1:10000) {
        ...
        interaction.coefficient.estimates[j] <- summary(shuffled.model)$coefficients['enhancer.1.indicator.vector:enhancer.2.indicator.vector', 'Estimate']

    }

    # write coefficient estimates to output file
    write.csv(interaction.coefficient.estimates, paste0('/iblm/netapp/home/karthik/GLiMMIRS/gasperini_data/', '23_03_31_', enhancer.1, '_', enhancer.2, '_', gene, '_null_interaction_coefficient_estimates.csv'), row.names = FALSE)
}
